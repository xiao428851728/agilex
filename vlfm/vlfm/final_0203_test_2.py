...
# ==============================================================================
# ⚡️⚡️ 关键函数：直接提取全局 Frontier ⚡️⚡️
# ==============================================================================
def get_best_frontier_goal(policy_instance):
    """
    不依赖局部 waypoint，直接从 ValueMap 获取全局最优前沿点 (Best Frontier)。
    这是给 MoveBase 用的最完美目标点。
    """
    try:
        # 1. 找到 Mapper
        mapper = None
        ...
        # 2. 从 ValueMap 获取 best_frontier
        if mapper and hasattr(mapper, "value_map"):
            frontier = mapper.value_map.best_frontier # 这是一个 Tensor([x, y])
            
            if frontier is not None:
                f_cpu = frontier.cpu().numpy().flatten()
                
                # 排除 (0,0) 的情况（除非真就在原点，但通常意味着未初始化）
                if abs(f_cpu[0]) > 0.01 or abs(f_cpu[1]) > 0.01:
                    return True, float(f_cpu[0]), float(f_cpu[1])
                    
    except Exception as e:
        print(f"⚠️ Frontier extraction failed: {e}")
    ...
    try:
        while True:
            ...
            with torch.no_grad():
                ...
                # ✅ 修复：正确提取 discrete_action 和 rnn_states
                discrete_action = 0 # 默认为 STOP
                
                if isinstance(action_data, tuple):
                    # 如果是元组 (actions, rnn_states)
                    actions_tensor, rnn_hidden_states = action_data
                    discrete_action = actions_tensor[0].item() # 提取动作索引 (0:Stop, 1:Fwd, 2:Left, 3:Right)
                else:
                    # 如果是 PolicyActionData 对象
                    rnn_hidden_states = action_data.rnn_hidden_states
                    discrete_action = action_data.actions[0].item()

                # 更新 prev_actions (这对 RNN 很重要)
                prev_actions.copy_(torch.tensor([[discrete_action]], device=DEVICE, dtype=torch.long))
                masks.fill_(True)

            # ==========================================================
            # ⚡️⚡️ 决策核心：优先使用全局 Frontier ⚡️⚡️
            # ==========================================================
            
            # A. 尝试获取全局目标
            has_goal, tx, ty = get_best_frontier_goal(policy)
            
            final_action = discrete_val # 默认保留 Policy 建议的动作 (Turn/Stop 等)
            
            if has_goal:
                # 如果有全局目标，覆盖 Action 为 FORWARD (1)，告诉 Robot 处于活动导航状态
                final_action = 1 
                print(f"🎯 Best Frontier: ({tx:.2f}, {ty:.2f}) -> MoveBase")
            else:
                # 如果没有目标 (通常是初始化中)，保留 discrete_val
                # 如果是 Action 2/3 (Left/Right)，Robot 会执行原地旋转来更新地图
                print(f"🔄 Init/Search Mode: Action {discrete_val}")
                tx, ty = 0.0, 0.0
            ...
            # ==========================================================
            # ⚡️ MODIFIED: 发送协议更改 (10字节: Action + Flag + X + Y)
            # ==========================================================
            
            # 1. 发送 Action + Goal
            # <B: ActionID (unsigned char), B: Flag, f: x, f: y
            packet = struct.pack("<BBff", discrete_action, int(has_goal), float(tx), float(ty))
            conn.sendall(packet)
