...
TARGET_SIZE = (224, 224)
TARGET_OBJECT_ID = 5
TARGET_NAME = "couch"   # change to your actual goal object name if needed
...
def get_nav_active_from_policy(policy_instance, has_goal: bool):
    """
    Minimal-change stage gate:
    Prefer the explicit state you added inside BaseObjectNavPolicy.act():
      self.server_nav_active = (mode == "navigate")
      self.server_mode = mode
    Fall back to has_goal if those fields are not available.
    """
...
while True:
    ...
    try:
        while True:
            ...
            obj_meta["nav_active"] = get_nav_active_from_policy(policy, has_goal)
