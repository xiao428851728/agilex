...
class BaseObjectNavPolicy(BasePolicy):
    ...
    def __init__(
        ...
        coco_threshold: float = 0.10,
        non_coco_threshold: float = 0.10,
        *args: Any,
        **kwargs: Any,
    ) -> None:
        super().__init__()
    ...
    def act(
        ...
        if goal is not None:
            print(f"Location of target object: {goal[:2]}")
        ...
        if not self._done_initializing:  # Initialize
            ...
            mode = "navigate"
            self.server_target_proxy = goal[:2]
...
class VLFMConfig:
    ...
    object_map_erosion_size: int = 2 #从5减少到2，降低检测阈值
