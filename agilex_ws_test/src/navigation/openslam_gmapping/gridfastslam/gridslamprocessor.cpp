...
//      std::cout<<!m_count<<","<<(m_angularDistance>=m_angularThresholdDistance)<<","<<(m_angularDistance>=m_angularThresholdDistance)<<","<<(period_ >= 0.0 && (reading.getTime() - last_update_time_) > period_)<<","<<std::endl;
//      std::cout<<""<<std::endl;

      std::cout<<">>>>>>>total_time="<<reading.getTime()-last_update_time_<<"s"<<std::endl;
  ...
  std::cout<<"<<<<<<<scan_time="<<(end-start)/1000000<<"s"<<std::endl;
//  start=clock();
	if (m_outputStream.is_open()){
